<template>
    ...
    <div class="yd-common-select-tree">
        <div class="bg-skin tree-title border-skin"><h3>选择班级</h3></div>
        <div class="tree-content" :style="{'maxHeight':(this.$store.state.minClientHeight*0.5)+'px'}">
            <el-tree
                    :data="treeData"
                    :props="defaultProps"
                    show-checkbox
                ...
                <div class="custom-tree-node" slot-scope="{node,data }">
                    <span>
                        ...
                        <i v-if="data.id.substring(0,1) == 'b'" class="yd icon-tree-classRoom"></i>
                        {{data.label}}
                    </span>
                </div>
            </el-tree>
...
<script>
    ...
    export default {
        ...
        methods:{
            getTreeData() {
                this.treeData = []
                this.$GET('/departmentInfo/queryTreeByZy',{classInfo:1}).then((res) => {
                    this.treeData = res.data
                    // console.log(this.treeData)
                    if (this.bjid != '') {
                        //赋值传递过来的id 给tree
                        let ksidString = this.bjid.toString() //转换科室号为字符串
                        let ksidArry = ksidString == ''? [] :ksidString.split(',')  //转换课室号为数组
                        ksidArry.forEach(item => {
                            this.checkedId.push('b' + item)
                        })
                        //赋值传递归来的id 的父级 展开
            ...
            submitTree() { //提交教研组
                ...
                if (treeData.length > 0) {
                    ...
                    this.$emit('checkedBjId',checkedId.join(','),name.join(','))
                }
            },
        }
    }
