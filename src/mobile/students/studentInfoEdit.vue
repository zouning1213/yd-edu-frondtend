...
<script>
  ...
  import VanFieldSelectPicker from '../teacher/components/VanFieldSelectPicker'
  ...
  export default {
    ...
    methods: {
      ...
      //提交修改
      subitEditInfo(){
        // this.ruleForm.jgm = JSON.stringify((this.ruleForm.znjgm))
        this.ruleForm.jgm = this.ruleForm.znjgm? JSON.stringify((this.ruleForm.znjgm)):''
        // console.log(this.ruleForm)
        this.$POST('/studentInfo/update',this.ruleForm).then((res) => {
            if (res.code == 200) {
                //  console.log("修改成功")
                 this.getUserInfo()
                 this.$router.go(-1)
