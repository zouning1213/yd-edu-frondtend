<template>
  <div class="entering-score-list-wrapper">
    ...
    <div class="head-filtrate-box">
      ...
      <van-tabs v-model="active" animated swipeable @click="tabClick">
          <van-tab v-for="(item,index) in nfList" :key="index" :name="item.id" :title="item.name">
          </van-tab>
      </van-tabs>
    </div>
    ...
    <div class="entering-score-list-box">
      <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
        <div class="entering-score-item" v-for="(item,index) in (active==1?tableList:tableListTwo)" :key="index">
          <div class="item-head">
            <span class="icon" :class="['gray',{'green':item.lrztm % 3 == 0},{'red':item.lrztm % 2 == 0}]"><label
              class="tag-type">{{active==1?item.lrzt:'不用审核'}}</label></span>
          ...
          <div class="item-des">
            <div>选择学期：2018-2019年学期</div>
             <router-link v-show="active==1" tag="div"  :to="{name:'entering_score_detail',query:{cjlrh:item.cjlrh,fromwhere:'kscjlr'}}">
             ...
             <router-link v-show="active==2" tag="div"  :to="{name:'entering_score_detail',query:{cjlrh:item.cjlrh,fromwhere:'pscjlr',bjid:item.bjid,kcid:item.kcid}}">
